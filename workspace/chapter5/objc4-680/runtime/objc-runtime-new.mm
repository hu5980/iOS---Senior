...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    //主要针对 分类中实现了 相关内存管理方法 会覆盖宿主类的相应 的内存管理方法
    prepareMethodLists(cls, mlists, mcount, NO, fromBundle);
    
    /*
     rw 代表类
     methods 代表leukemia的方法列表
     attachLists 方法的含义是  将含有mcount 个元素的mlists拼接到rw的methods上
     */
    
    rw->methods.attachLists(mlists, mcount); //将处理过的二维数组的分类添加到 相应的宿主类 的方法数组上面
    free(mlists);
